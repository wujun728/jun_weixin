Ext.define("app.view.articleCategory.ArticleCategoryController", {
	extend : "Ext.app.ViewController",
	alias : "controller.articleCategory",
	mixins : {
	...
	onAdd : function(button) {
		var code = "articlecategoryform";
		var tab = this.findTabByCode(code);
		if (!tab) {
			var view = Ext.create("app.view.articleCategory.ArticleCategoryForm", {
				id : code,
				closable : true,
				title : "新建文章分类",
				iconCls : "fa fa-user"
	...
	onRowEdit : function(tree, rowIndex, colIndex) {
		...
		var code = "articlecategoryform" + item.id;
		var tab = this.findTabByCode(code);
		if (!tab) {
			var view = Ext.create("app.view.articleCategory.ArticleCategoryForm", {
				id : code,
	...
	onRowDelete : function(tree, rowIndex, colIndex) {
		...
		Ext.Msg.confirm("提示", "您确定要删除【" + menu.get("name") + "】？", function(choice) {
			if (choice === "yes") {
				...
				Ext.Ajax.request({
					url : Ext.ctx + "/articleCategory/delete",
					params : {
						id : menu.id
					},
					method : "POST",
	...
	onFormSave : function(button) {
		var formPanel = button.up("articlecategoryform");
	...
	onDelete : function(button) {
		var articleCategoryTree = button.up("articleCategory");
		var selection = articleCategoryTree.getSelection();
