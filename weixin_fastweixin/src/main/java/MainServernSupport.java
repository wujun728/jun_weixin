...
public class MainServernSupport {
	...
	public static String getAESKey() {
		return "X6VAYFjUuqPmIDsG588kjkD5xfZNSyH4FpT0CVSaohO";
	}
    ...
    public void bindServer(HttpServletRequest request,
        ...
        PrintWriter pw = null;
        LOG.error("signature",request.getParameter("signature"));
        LOG.error("timestamp",request.getParameter("timestamp"));
        LOG.error("nonce",request.getParameter("nonce"));
        LOG.error("echostr",request.getParameter("echostr"));
        try {
        ...
        if (StrUtil.isBlank(request.getParameter("echostr")) || StrUtil.isBlank(request.getParameter("signature")) || StrUtil.isBlank(request.getParameter("timestamp")) ||
        ...
        try {
            ...
            String echoStr =
                pc.verifyUrl(request.getParameter("signature"),
        	...
        	if (StrUtil.isNotBlank(e.getLocalizedMessage())) {
        	      LOG.error(e.getLocalizedMessage());
			}
            LOG.error("signature",request.getParameter("signature"));
            LOG.error("timestamp",request.getParameter("timestamp"));
