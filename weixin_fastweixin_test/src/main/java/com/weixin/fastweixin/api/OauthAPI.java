...
public class OauthAPI extends BaseAPI {
	...
	public String getOauthPageUrl(String redirectUrl, OauthScope scope, String state) {
		...
		stringBuilder.append("appid=").append(this.config.getAppid()).append("&redirect_uri=").append(url).append("&response_type=code&scope=")
	...
	public OauthGetTokenResponse getToken(String code) {
		...
		String url = BASE_API_URL + "sns/oauth2/access_token?appid=" + this.config.getAppid() + "&secret=" + this.config.getSecret() + "&code=" + code
	...
	public OauthGetTokenResponse refreshToken(String refreshToken) {
		...
		OauthGetTokenResponse response = null;
		String url = BASE_API_URL + "sns/oauth2/refresh_token?appid=" + this.config.getAppid() + "&grant_type=refresh_token&refresh_token="
				+ refreshToken;
		BaseResponse r = executeGet(url);
		String resultJson = isSuccess(r.getErrcode()) ? r.getErrmsg() : r.toJsonString();
		response = JSONUtil.toBean(resultJson, OauthGetTokenResponse.class);
