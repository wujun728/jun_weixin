...
import com.weixin.fastweixin.company.api.response.GetOauthUserInfoResponse;
import com.weixin.fastweixin.company.api.response.GetQYUserInfo4DepartmentResponse;
import com.weixin.fastweixin.company.api.response.GetQYUserInfoResponse;
import com.weixin.fastweixin.company.api.response.GetQYUserInviteResponse;
...
public class QYUserAPI extends QYBaseAPI {
	...
	public GetQYUserInfo4DepartmentResponse simpleList(Integer departmentId, boolean isLoop, Integer status) {
		...
		String url = BASE_API_URL + "cgi-bin/user/simplelist?access_token=#&department_id=" + departmentId + "&fetch_child=" + (isLoop ? 0 : 1)
				+ "&status=" + status;
		BaseResponse r = executeGet(url);
		String resultJson = isSuccess(r.getErrcode()) ? r.getErrmsg() : r.toJsonString();
	...
	public GetQYUserInfo4DepartmentResponse getList(Integer departmentId, boolean isLoop, Integer status) {
		GetQYUserInfo4DepartmentResponse response;
		String url = BASE_API_URL + "cgi-bin/user/list?access_token=#&department_id=" + departmentId + "&fetch_child=" + (isLoop ? 0 : 1) + "&status="
				+ status;
		BaseResponse r = executeGet(url);
		String resultJson = isSuccess(r.getErrcode()) ? r.getErrmsg() : r.toJsonString();
		response = JSONUtil.toBean(resultJson, GetQYUserInfo4DepartmentResponse.class);
