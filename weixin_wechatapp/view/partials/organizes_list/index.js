...
Component({
  ...
  data: {
    ...
    sysWidth: 320,//图片大小
    positionTab: '',
    OrganizeshowWay: 2, // SpaceshowWay列表显示方法 (默认显示地图)
  ...
  ready: function () {
    ...
    wx.getLocation({
      ...
      success: function (res) {
        ...
        if (options.organizesTypeId) {
          that.setData({ positionTab: options.organizesTypeId })
          options.categoryId = options.organizesTypeId
          that.bindTypeItem(options.organizesTypeId)
        }
  ...
  methods: {
    ...
    hiddenProInfo(e) {
      console.log(e)
      this.setData({ organizesDetail: null })
    },
    /* 点击分类大项 */
    ...
    /* 查找商品 */
    getSearchOrganizeName: function (params) {
      console.log("====getSearchOrganizesName====", params)
      var that = this
      that.getData(params)
    },
    /* 商品显示方法 */
