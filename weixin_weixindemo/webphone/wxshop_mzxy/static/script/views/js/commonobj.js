define(["jquery"],function(require){
    return commonObj = {
        ...
        getData:function(pagenumber){
            $.ajax({
                type:"get",
                url:"/webphone/wxshop_mzxy/static/script/test.json",
                data:{
        ...
        insertDiv:function(json){
            ...
            for(var i=0;i<showlength;i++){
                ...
                '<div class="productcontent fr">'+
                    ...
                    '<p class="pdes pl10">广联达建设工程材料管理软件满足项目经理、商务经理、材料员、库管员、材料会计在同一平台上的协同应用需求。</p>'+
                    '<p class="pprice pl10">价格：<span class="green">￥5000</span></p>'+
                '</div> </a> </li>'+
                    '<li><a href="#">'+
                    '<div class="triangle-topleft rexiao" style="border-top-color:#d50806"></div>'+
                    ...
                    '</div></a></li>'+
                '<li><a href="#">'+
                    '<div class="triangle-topleft xiangou" style="border-top-color:#ff7301"></div>'+
        ...
        scrollHandler:function(){
            ...
            var scrollT=$(window).scrollTop();   //滚动条
            var winheight=$(window).height();

            if(parseInt(winheight)+parseInt(scrollT)+50>=parseInt(pageH) && commonObj.ajaxstatus){
        ...
        reducenums:function(){
           ...
           commonObj.setTotal();
            ...
            $(".cartlist li").each(function(){
                s+=parseInt($(this).find('.itnums').val())
                    *parseFloat($(this).find('.productoperate i').text());
                m+=parseInt($(".cartlist li").find('.itnums').val())
            });
            $("#totalMoney").html(s.toFixed(2));

            localStorage.setItem("money",s.toFixed(2));
            localStorage.setItem("account",m);

        },subtotal:function(){

            //var subtotal=0;
           // subtotal=parseFloat($(".productoperate i").html())*parseInt($("input[type=number]").val());
                ...
                $obj.css({"left":x,"top":y}).animate({"left":X,"top":Y-80},500,function(){
                    $obj.stop(false,false).animate({"top":Y-20,"opacity":0},500,function(){
                        $obj.fadeOut(300,function(){
                            ...
                            $(".cartnums").show();
                            localStorage.setItem("atvt",num+number);
                        });
                    });
                })
            }
