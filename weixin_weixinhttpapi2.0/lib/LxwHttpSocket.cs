...
namespace HttpSocket
{
    class LxwHttpSocket
    {
        ...
        LxwResponse SendSSLRequest(TcpClient client, byte[] sendBody, LxwRequestHeader sendHeader,bool write=false)
        {
            ...
            if (write)
            {
                FileStream fs = new FileStream(@"C:\Users\Administrator\Desktop\ssl" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".data", FileMode.CreateNew);
        ...
        LxwResponse SendNoSSLRequest(TcpClient client, byte[] sendBody, LxwRequestHeader sendHeader, bool write = false)
        {
            ...
            if (write)
            {
                FileStream fs = new FileStream(@"C:\Users\Administrator\Desktop\nossl" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".data", FileMode.CreateNew);
                fs.Write(sendHeader.HeaderByte, 0, sendHeader.HeaderByte.Length);
                fs.Write(LINE, 0, LINE.Length);
                fs.Write(LINE, 0, LINE.Length);
                if (sendBody != null)
